...
def risk_pass_rate():
    ...
    try:
        ...
        # 2018年12月3日 当日的注册数>50的情况下，注册放款率或者审核通过率=0，level=1；9:00~14:00：注册放款率<3.5%或者审核通过率<10%, level=2；14:00以后注册放款率<5%或者审核通过率<10%, level=2
        if not count:
        ...
        pass_rate = 0.035 if time_util.gettime(14) + 10 >= time.time() else 0.05
